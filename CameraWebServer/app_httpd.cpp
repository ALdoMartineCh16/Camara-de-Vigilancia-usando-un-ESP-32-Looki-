...
const char* login_password = "1234";
...
static esp_err_t index_handler(httpd_req_t *req) {
  ...
  if (httpd_req_get_hdr_value_str(req, "Authorization", auth_header, sizeof(auth_header)) == ESP_OK) {
    if (strstr(auth_header, "Basic ") == auth_header) {
      char *credentials = auth_header + 6;  // Salta "Basic "
      // Base64 de "admin:1234" = YWRtaW46MTIzNA==
      if (strcmp(credentials, "YWRtaW46MTIzNA==") != 0) {
        httpd_resp_set_status(req, "401 Unauthorized");
        httpd_resp_set_hdr(req, "WWW-Authenticate", "Basic realm=\"ESP32-CAM\"");
        httpd_resp_send(req, NULL, 0);
        return ESP_OK;
  ...
  // --- Si pasa la autenticación, continúa como siempre ---
